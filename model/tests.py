...
class MetricsTests(unittest.TestCase):
	...
	def test_wire_length(self):
		...
		self.assertTrue(metrics.wire_length(boards, b, topology.NORTH), 2.0)
		# South wire (which reaches the chip above-right)
		self.assertTrue(metrics.wire_length(boards, b, topology.SOUTH), 2.0**0.5)
		
		# Now while giving a non-applicable wire_offsets
		# North-East wire (which reaches the chip above-right)
		self.assertTrue(metrics.wire_length(boards, b, topology.NORTH_EAST, wire_offsets),
			2.0**0.5)
		# South-West wire (which reaches the chip above)
		self.assertTrue(metrics.wire_length(boards, b, topology.SOUTH_WEST, wire_offsets),
			2.0)
		
		# Now while giving a applicable wire_offsets
		# North wire (which reaches the chip above)
		self.assertTrue(metrics.wire_length(boards, b, topology.NORTH, wire_offsets),
			3**0.5)
		# South wire (which reaches the chip above-right)
		self.assertTrue(metrics.wire_length(boards, b, topology.SOUTH, wire_offsets),
			2.0)
