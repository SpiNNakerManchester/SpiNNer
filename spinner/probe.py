...
class WiringProbe(object):
	...
	def _assign_link_ids(self):
		...
		# random mask is XOR'd into every ID. As a final complication, we must never
		# allocate the key 0xFFFF since this value causes the link to send random
		# ID values (the default behaviour on reset, and designed to reduce the
		# interference generated by the system) to ensure this, the mask always has
		# its top bit clear meaning that as long as the number of link endpoints is
		# < 32,768 everything should just work. (The 1-million core machine will
		# have 7,200 link end points so this is a fairly safe limitation.)
