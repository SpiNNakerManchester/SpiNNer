"""
Tools for producing diagrams which show the network topology overlaid with
board boundaries and physical locations.
"""
...
def _draw_edge(ctx, x, y, edge):
    ...
    # The transformation to use when drawing hexagons such that they sit on an
    # x,y grid.
    ctx.save()
    x_scale = 1.0/(2.0*sin(D2R(60)))
    y_scale = 1.0/(1.0+cos(D2R(60)))
    ctx.translate((0.25*tan(D2R(60))) * x_scale, 0.5*sin(D2R(30)) * y_scale)
    ctx.move_to(x, y)
    ctx.transform(cairo.Matrix(1.0, 0.0, -cos(D2R(60)), 1.0))
...
def _fill_hex(ctx, x, y):
    """
    Fill a hexagon whose bottom-left corner is at
    (x,y) using whatever style is currently set.
    """
...
def _draw_text(ctx, text, align_point=0.0, size=1.0,
    ...
    x, y, w, _h, _w, _h = ctx.text_extents(text)
...
def _draw_board(ctx, x, y, width_chips, height_chips, btm_left_chip,
                board_label, board_colours):
    """
    Draw a board at the given position onto the given context in a system of
    the given width/height in chips. If specified a label will be added to the
    bottom-left chip and the board as a whole. Edges of chips at the edge of
    the board will be drawn differently to indicate which HSS link they will
    use.
    """
    # Draw the chips
    hexagon = set(hexagon_zero())
    max_x = max(x_ for (x_, _y) in hexagon)
    max_y = max(y_ for (_x, y_) in hexagon)
    for dx, dy in hexagon:
        ...
        colour = board_colours.get((x_, -y_))
    ...
    _draw_text(ctx, btm_left_chip, align_point=0.0,
    ...
    for x_, y_ in positions:
        ...
        for line in lines:
            _draw_text(ctx, line, align_point=0.5, size=BOARD_LABEL_SIZE,
...
def _draw_coords(ctx, width, height):
    """
    Given a context and a width and height in chips, draw numbered ticks along
    tje edges of the diagram.
    """
    for num, rotation, offset, alignment in (
            (height, 0.0, -1 - COORD_LABEL_OFFSET, 1.0),  # Left
        ...
        for i in range(num):
            _draw_text(ctx, str(i), align_point=alignment,
                       size=COORD_LABEL_SIZE, rgba=COORD_LABEL_COLOUR)
...
def draw_machine_map(ctx, image_width, image_height,
                     width, height,
                     hex_boards, cabinetised_boards, board_colours=None,
                     include_boards=None):
    """
    Given a Cairo context (and the size of the rectangle to draw in), draws a
    map of the machine. Takes the width and height of the system in triads,
    hex_boards (mapping 3D hexagonal board coordinates to boards) and a
    cabinetised version of the same.
    """
    if board_colours is None:
        board_colours = dict()
    ...
    max_cabinet = None
    max_frame = None
    if cabinetised_boards:
        max_cabinet = max(c for (_b, (c, _f, _b)) in cabinetised_boards)
        max_frame = max(f for (_b, (_c, f, _b)) in cabinetised_boards)
    ...
    for board, board_pos in hex_boards:
        ...
        if include_boards is not None and (x, y) not in include_boards:
            continue
        label = ""
        if cabinetised_boards:
            c, f, b = b2c[board]
